export function getCrossExPrompt(topic, userInput, messages, division) {
  const convo = messages
    ...
    If they did not say anything, just comment that we will move on to next section since they did not provide a constructive.
...
export function getCrossExResponsePrompt(topic, userInput, messages, division){
    ...
    The opponent just said:
    "${userInput}"
    If they did not say anything, just comment that we will move on to next section since they did not provide a questions.
...
export function getJudgePrompt(topic, messages){
    ...
    - Explain which side had the stronger overall case, and the main reasons for your decision
    - Evaluate argument strength, refutation, organization, clarity, and persuasiveness
    - Evaluate evidence quality AND quantity, look at relevance and publication
    - Reference specific moments or arguments from the debate
    - Do not hallucinate points made, be brutally honest and just use info from the provided transcript
    - Be fair, clear, and concise

    Format your response something like this:
